...
Cartridge::Cartridge(const uint8_t* irom_data, size_t irom_size)
{
    ...
    rom_data = irom_data;
    rom_size = irom_size;
    header = (cartridge_header*)rom_data;
...
bool Cartridge::cpuReadBlock(uint16_t addr, uint32_t size, uint8_t* data)
{
    return mapper.vtable->cpuReadBlock(&mapper, addr, size, data);
}
...
IRAM_ATTR const uint8_t* Cartridge::ppuReadPtr(uint16_t addr)
{
...
IRAM_ATTR const uint8_t* Cartridge::loadPRGBank(uint16_t size, uint32_t offset)
{
    //memcpy(prog, rom_data + prg_base + offset, size);
    //return prog;
    return rom_data + prg_base + offset;
}

IRAM_ATTR const uint8_t* Cartridge::loadCHRBank(uint16_t size, uint32_t offset)
{
    return rom_data + chr_base + offset;
}
