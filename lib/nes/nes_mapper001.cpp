...
struct Mapper001_state
{
    ...
    const uint8_t* ptr_16K_PRG_banks[4];
    const uint8_t* ptr_8K_CHR_bank;
    const uint8_t* ptr_4K_CHR_banks[2];
...
IRAM_ATTR bool Mapper001_cpuReadBlock(Mapper* mapper, uint16_t addr, uint32_t size, uint8_t* data)
{
    if (addr < 0x6000) return false;
    if (addr < 0x8000)
    {
        ...
        {
            data[i] = ((Mapper001_state*)mapper->state)->RAM[(addr & 0x1FFF) + i];
        }
        return true;
    }
    ...
    if (state->PRG_ROM_bank_mode < 2)
    {
        ...
        {
            ((uint32_t*)data)[i] = ((const uint32_t*)(state->ptr_16K_PRG_banks[bank]))[ (addr & 0x3FFF) / 4 + i];
        }
    }
    else
    {
    ...
    if (!(data & 0x80))
    {
        ...
        if (state->load_writes == 5)
        {
            ...
            case 1:
                ...
                {
                    if (state->number_CHR_banks == 0) state->ptr_8K_CHR_bank = state->cart->loadCHRBank(8*1024, (state->CHR_bank_0 & 0x1E) * 8*1024);
                    else state->ptr_8K_CHR_bank = getBank(&state->CHR_8K_cache, state->CHR_bank_0 & 0x1E, Mapper::ROM_TYPE::CHR_ROM);
                }
                else
                {
                    if (state->number_CHR_banks == 0) state->ptr_4K_CHR_banks[0] = state->cart->loadCHRBank(4*1024, state->CHR_bank_0 * 4*1024);
            ...
            case 2:
                ...
                {
                    if (state->number_CHR_banks == 0) state->ptr_4K_CHR_banks[1] = state->cart->loadCHRBank(4*1024, state->CHR_bank_1 * 4*1024);
...
IRAM_ATTR const uint8_t* Mapper001_ppuReadPtr(Mapper* mapper, uint16_t addr)
{
...
Mapper createMapper001(uint8_t PRG_banks, uint8_t CHR_banks, Cartridge* cart)
{
    ...
    Mapper001_state* state = &mapper1_state;//new Mapper001_state;
    ...
    state->RAM = mapper1_ram;//(uint8_t*)malloc(8*1024);
